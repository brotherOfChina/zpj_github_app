...
class Address{
  ...
  static search(q,sort,order,type,page,[pageSize=Config.PAGE_SIZE]){
    ...
    return "${host}search/repsositories?q=$q&sort=$sort&order=$order&page=$page&per_page=$pageSize";

  }
  ///搜索 topic tag
  static searchTopic(topic){
    return "${host}search/repositoreis?q=topic:$topic&sort=stars&order=desc";
  }
  ///用户的仓库  get
  ...
  static getReposEvent(reposOwner,reposName){
    return "${host}repos/$reposOwner/$reposName/events";
  }
  ///仓库fork get
  ...
  static getReposWatcher(reposOwner,reposName){
    return "${host}repos/$reposOwner/$reposOwner/subscribers";
